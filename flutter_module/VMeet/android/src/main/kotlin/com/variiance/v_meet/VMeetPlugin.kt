...
/** VMeetPlugin */
public class VMeetPlugin() : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    // The EventChannel for broadcasting VMeetEvents to Flutter
    ...
    override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        channel = MethodChannel(flutterPluginBinding.binaryMessenger, VMeet_METHOD_CHANNEL)
        channel.setMethodCallHandler(this)

        eventChannel = EventChannel(flutterPluginBinding.binaryMessenger, VMeet_EVENT_CHANNEL)
        eventChannel.setStreamHandler(VMeetEventStreamHandler.instance)
    }
    ...
    companion object {
        ...
        fun registerWith(registrar: Registrar) {
            val plugin = VMeetPlugin(registrar.activity())
            val channel = MethodChannel(registrar.messenger(), VMeet_METHOD_CHANNEL)
            channel.setMethodCallHandler(plugin)


            val eventChannel = EventChannel(registrar.messenger(), VMeet_EVENT_CHANNEL)
            eventChannel.setStreamHandler(VMeetEventStreamHandler.instance)
        }

        const val VMeet_PLUGIN_TAG = "V_MEET_PLUGIN"
        const val VMeet_METHOD_CHANNEL = "v_meet"
        const val VMeet_EVENT_CHANNEL = "v_meet_events"
        const val V_MEETING_CLOSE = "V_MEETING_CLOSE"
    }
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        Log.d(VMeet_PLUGIN_TAG, "method: ${call.method}")
        Log.d(VMeet_PLUGIN_TAG, "arguments: ${call.arguments}")
    ...
    private fun joinMeeting(call: MethodCall, result: Result) {
        ...
        if (call.argument<String?>("userAvatarURL") != null) {
         userInfo.avatar = URL(call.argument<String?>("userAvatarURL"))
        } 
        ...
        // Add config into options, reading given Map
        if (call.argument<HashMap<String, Any>?>("config") != null) {
            val config = call.argument<HashMap<String, Any>>("config")
            config!!.forEach { (key, value) ->
                if (value is Boolean) {
                    val boolVal = value.toString().toBoolean()
                    optionsBuilder.setConfigOverride(key, boolVal)
                } else if ( value is Int) {
                    val intVal = value.toString().toInt()
                    optionsBuilder.setConfigOverride(key, intVal)
                } else if ( value is Array<*> ){    
                 optionsBuilder.setConfigOverride(key, value as? Array<String>)
                } else if (value is String){
                 optionsBuilder.setConfigOverride(key, value)
                }
            }
        }
        ...
        if (call.argument<HashMap<String, Any>?>("featureFlags") != null) {
            ...
            featureFlags!!.forEach { (key, value) ->
              Log.d(VMeet_PLUGIN_TAG, "featureFlags:  ${key} ====  ${value}")
                if (value is Boolean) {
                    val boolVal = value.toString().toBoolean()
                    ...
                    optionsBuilder.setFeatureFlag(key, stringVal)
                }
            }
        }
